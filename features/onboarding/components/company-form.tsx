...
export function CompanyForm({ status, userId }: { status: string | null; userId: string }) {
  ...
  const handleNext = async () => {
    ...
    } else if (step === 5) { // Assuming step 5 is file upload for company and step 6 is UBO
       isValid = true // Can manually validate docs length
    }
  ...
  async function onSubmitStep5(e: React.MouseEvent) {
    ...
    try {
      ...
      toast.success('Datos base enviados. Continúa con los UBOs.')
      setStep(6)
    } catch {
       toast.error('Ocurrió un error al enviar datos de empresa')
    }
  }
  ...
  async function onFinalUBOSubmit(data: CompanyOnboardingValues) {
    if (status !== 'submitted' && status !== 'under_review' && status !== 'waiting_ubo_kyc' && status !== 'needs_changes') {
       // Should have been submitted softly on step 5, but as a fallback
    }
    ...
    try {
      ...
      toast.success('Declaración UBO enviada con éxito')
  ...
  return (
    <Form {...form}>
      <form className="space-y-6" onSubmit={form.handleSubmit(onFinalUBOSubmit)}>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField control={form.control} name="registration_number" render={({ field }) => (
                <FormItem><FormLabel>Núm. Registro</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )} />
              ...
              <FormField control={form.control} name="incorporation_date" render={({ field }) => (
               ...
               <FormField control={form.control} name="country_of_incorporation" render={({ field }) => (
                <FormItem><FormLabel>País Const.</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )} />
               <FormField control={form.control} name="business_description" render={({ field }) => (
                <FormItem className="col-span-2"><FormLabel>Descripción Actividad</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )} />
            </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            <h2 className="text-xl font-medium">Dirección Legal y Representante</h2>
            <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
               <FormField control={form.control} name="purpose" render={({ field }) => (
                <FormItem><FormLabel>Propósito de cuenta</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )} />
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="border p-4 rounded bg-muted/20">
                <FormLabel className="block mb-2">Constitución / Registro Empresa</FormLabel>
        ...
        {step === 6 && (
          <div className="space-y-6">
             <div className="flex justify-between items-center">
              <h2 className="text-xl font-medium">Declaración de Socios (UBO)</h2>
             ...
             <div className="flex justify-between pt-4 mt-8 border-t">
               ...
               <Button type="submit" disabled={isUploading || form.formState.isSubmitting}>
                 {form.formState.isSubmitting ? 'Enviando UBOs...' : 'Finalizar Revisión Empresarial'}
               </Button>
             </div>
          </div>
        )}
